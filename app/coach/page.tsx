...
function intakeDetail(row: any) {
  const goals = Array.isArray(row.primary_goals) && row.primary_goals.length ? row.primary_goals.join(', ') : 'Goals ready for review'
  const schedule = Array.isArray(row.available_days) && row.available_days.length ? ` · ${row.available_days.join(', ')}` : ''
  const location = row.training_location ? ` · ${row.training_location}` : ''
  const success = row.success_definition ? ` — ${row.success_definition}` : ''
  return `${goals}${schedule}${location}${success}`
}
...
export default async function CoachPage() {
  ...
  let queue: QueueItem[] = [
    { name: 'New client intake', detail: 'Goals, schedule, equipment, training history, recovery, and coaching preferences appear here.', status: 'Needs review' },
    { name: 'Program build queue', detail: 'Create a personalized program by selecting reusable exercises and setting client-specific prescriptions.', status: 'Build next' },
  ]
  let notices: Notice[] = [
    { title: 'Weekly check-in submitted', body: 'Client check-ins appear here immediately after submission.', age: 'Notification preview' },
    { title: 'Intake submitted', body: 'New onboarding submissions create an owner notification too.', age: 'Notification preview' },
  ]
  ...
  if (supabase) {
    ...
    const [clients, intakes, programs, notifications] = await Promise.all([
      ...
      supabase.from('programs').select('*', { count: 'exact', head: true }).in('status', ['draft', 'active']),
    ...
    const { data: intakeRows } = await supabase
      .from('intakes')
      .select('id,primary_goals,success_definition,available_days,training_location,submitted_at,profiles!intakes_client_id_fkey(full_name,email)')
      .eq('status', 'submitted')
      .order('submitted_at', { ascending: false })
      .limit(5)

    if (intakeRows?.length) {
      queue = intakeRows.map((row: any) => ({
        name: row.profiles?.full_name || row.profiles?.email || 'Client intake',
        detail: intakeDetail(row),
        status: 'Needs review',
      }))
    }
    ...
    if (notificationRows?.length) {
      notices = notificationRows.map((row: any) => ({
        title: row.title,
        body: row.body || '',
        age: new Date(row.created_at).toLocaleDateString(),
      }))
    }
  }
          ...
          <div className="notification-list">
            {notices.map((notice, index) => (
              <div className="notification-row" key={`${notice.title}-${index}`}>
                ...
                <span className="status-chip">{notice.age}</span>
              </div>
            ))}
          </div>
          <p style={{ opacity: .48, fontSize: 11, lineHeight: 1.7, marginTop: 18 }}>
            Database triggers create an in-app owner notification whenever an intake or weekly check-in is submitted.
          </p>
        </section>
      </div>
    </main>
